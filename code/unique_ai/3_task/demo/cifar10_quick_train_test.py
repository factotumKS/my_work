...
with tf.Session(config = tf.ConfigProto(log_device_placement = True)) as sess:
	...
	for i in range(Train_steps):
		...
		# test
		if i == Train_steps-1:
			randidx_test = np.random.randint(len(test_x), size=10000)
			batch_xs_test = test_x[randidx_test]
			batch_ys_test = test_y[randidx_test]
			accuracy_batch = sess.run(accuracy,feed_dict = {
				x:batch_xs_test,
				y_:batch_ys_test,
				keep_prob:1.0
			})
			print('the accuracy on test data is %g'%(accuracy_batch))
