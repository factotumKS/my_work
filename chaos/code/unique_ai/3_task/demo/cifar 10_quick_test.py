...
def add_full_layer(input_x,layer_numble,node_num,alpha = 1,activation_function = None):
	...
	tf.summary.histogram(layer_name+'/input',input_x)
...
with tf.name_scope('input'):
	...
	tf.summary.histogram('/input',x_imgs)
...
with tf.Session(config = tf.ConfigProto(log_device_placement = True)) as sess:
	...
	for i in range(Train_steps):
		randidx = np.random.randint(len(test_x), size=1000)
		batch_xs = test_x[randidx]
		batch_ys = test_y[randidx]
		# train
		...
		print('after %d train steps the loss on the test batch data is %g and the accuracy on test batch data is %g'%(i,loss_batch,accuracy_batch))