...
def getweight(shape,llambda):
    # 生成一个变量
    var = tf.Variable(tf.random_normal(shape), dtype = tf.float32)
    # add_to_collection函数将这个新生成的变量的L2正则化损失项加入集合
    # 这个函数的第一个参数是集合的名字，第二个参数是要加入这个集合的内容
    tf.add_to_collection(
            'losses',tf.contrib.layers.l2_regularizer(llambda) (var))
    # 返回生成的变量
    return var
...
for i in range(1, n_layers):
    ...
    # 进入下一层之前将下一层的节点数作为当前新的节点数
    in_dimension = layer_dimension[i]

# 在这里只定义神经网络前向传播的同时已经将所有的L2正则化损失加入了图上的集合
